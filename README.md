...
- `Review profile PR`：收到 `credits-profiles` 在 trusted profile review 通過後送出的 repository dispatch 時，匯出 canonical Google Sheet，確認 profile PR 的 username 是否已出現在 `appearances.github_username`；符合條件時用 `CREDITS_PROFILES_SYNC_TOKEN` approve 並以 squash merge 合併，不符合時在 PR 留言提醒維護者調整或確認 appearances 資料。
