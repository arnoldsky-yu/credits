...
- `Export Sheets data`：手動觸發時使用 `GOOGLE_SERVICE_ACCOUNT_JSON` repository secret 匯出 canonical Google Sheet，執行 `pnpm data:validate`，上傳 `tmp/sheets-export/` artifact，並用 `SITCON Credits Assistant` GitHub App 直接 commit 到 `credits-profiles`，為缺少的 `people.github_username` 建立空白 profile template。
...
- `Review profile PR`：收到 `credits-profiles` 在 trusted profile review 通過後送出的 repository dispatch 時，匯出 canonical Google Sheet，確認 profile PR 的 username 是否已出現在 `appearances.github_username`；符合條件時用 `SITCON Credits Assistant` GitHub App approve 並以 squash merge 合併，不符合時在 PR 留言提醒維護者調整或確認 appearances 資料。

`CI` 不讀取 service account credentials、不連線 Google APIs，也不匯出 canonical Sheet。`Export Sheets data`、`Sync people helper` 和 `Review profile PR` 需要維護者先在 GitHub repository secrets 設定 `GOOGLE_SERVICE_ACCOUNT_JSON`；跨 repo 寫入、留言、approve 或 merge `credits-profiles` 另需安裝 `SITCON Credits Assistant` GitHub App，並設定 `SITCON_CREDITS_ASSISTANT_APP_CLIENT_ID` repository variable 與 `SITCON_CREDITS_ASSISTANT_APP_PRIVATE_KEY` repository secret。這個 GitHub App 應安裝在 `sitcon-tw/credits` 與 `sitcon-tw/credits-profiles`，不應使用維護者個人 token。workflow 產生的 commit author 會固定為 `SITCON Credits Assistant`，committer 會使用 `sitcon-credits-assistant[bot]` 的 noreply email。沒有必要 secret 時 workflow 會失敗而不會讀取任何本機 credentials。GitHub Pages 建置部署仍是後續工作；在對應 workflow 與 repository 設定完成前，不應描述為已上線。
